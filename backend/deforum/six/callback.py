...
class SamplerCallback(object):
    ...
    def __init__(self, args, root, mask=None, init_latent=None, sigmas=None, sampler=None, step_callback=None,
                  verbose=False):
    ...
    def view_sample_step(self, latents, path_name_modifier=''):
        if self.save_sample_per_step:
            ...
            for i, sample in enumerate(samples):
                ...
                print(os.path.join(self.paths_to_image_steps[i], fname))
        if self.show_sample_per_step:
            pass
            #samples = gs.models["sd"].linear_decode(latents)
    ...
    def k_callback_(self, args_dict):
        ...
        if self.step_callback is not None:
            self.step_callback(args_dict['denoised'], 1)
