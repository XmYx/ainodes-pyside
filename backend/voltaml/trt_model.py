...
import \
    pycuda.autoinit  # without this, "LogicError: explicit_context_dependent failed: invalid device context - no currently active context?"
    ...
    def __call__(self, model_inputs: list, timing=False):
        ...
        for i, model_input in enumerate(model_inputs):
            ...
            if NUMPY:
                ...
                if timing:
                    cuda.memcpy_dtod(self.inputs[i].device, model_input.data_ptr(),
                                     model_input.element_size() * model_input.nelement())
                else:
                    # for Torch GPU tensor it's easier, can just do Device to Device copy
                    cuda.memcpy_dtod_async(self.inputs[i].device, model_input.data_ptr(),
                                           model_input.element_size() * model_input.nelement(),
                                           self.stream)  # dtod need size in bytes
        ...
        if timing:
            ...
            self.context.execute_async_v2(bindings=self.bindings,
                                          stream_handle=self.stream.handle)  # v2 no need for batch_size arg
