...
def generate(args, root, frame = 0, return_latent=False, return_sample=False, return_c=False, step_callback=None, hires=None):
    ...
    if args.init_mse_scale != 0:
        if args.decode_method == "linear":
            mse_loss_fn = make_mse_loss(gs.models["sd"].linear_decode(gs.models["sd"].get_first_stage_encoding(gs.models["sd"].encode_first_stage(init_mse_image.to(root.device)))))
        else:
    ...
    if args.colormatch_scale != 0:
        ...
        if args.decode_method == "linear":
            ...
            colormatch_image = gs.models["sd"].linear_decode(gs.models["sd"].get_first_stage_encoding(gs.models["sd"].encode_first_stage(colormatch_image.to(root.device))))
            colormatch_image = colormatch_image.to('cpu')
