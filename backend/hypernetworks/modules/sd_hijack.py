...
class StableDiffusionModelHijack:
    ...
    def hijack(self, m):
        ...
        self.layers = flatten(gs.models["sd"])

    def undo_hijack(self, m):
        if type(gs.models["sd"].cond_stage_model) == FrozenCLIPEmbedderWithCustomWords:
            gs.models["sd"].cond_stage_model = gs.models["sd"].cond_stage_model.wrapped
...
class FrozenCLIPEmbedderWithCustomWords(torch.nn.Module):
    ...
    def tokenize_line(self, line, used_custom_terms, hijack_comments):
        ...
        for tokens, (text, weight) in zip(tokenized, parsed):
            ...
            while i < len(tokens):
                ...
                elif gs.comma_padding_backtrack != 0 and max(len(remade_tokens), 1) % 75 == 0 and last_comma != -1 and len(remade_tokens) - last_comma <= gs.comma_padding_backtrack:
                    last_comma += 1
        ...
        for tokens in batch_tokens:
            ...
            if tuple_tokens in cache:
                ...
                while i < len(tokens):
                    ...
                    mult_change = self.token_mults.get(token) if gs.enable_emphasis else None
    ...
    def forward(self, text):
        use_old = gs.use_old_emphasis_implementation
        if use_old:
