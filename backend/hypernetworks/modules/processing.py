...
from backend.hypernetworks.modules import devices, prompt_parser, masking, sd_samplers, lowvram, generation_parameters_copypaste
from backend.hypernetworks.modules.sd_hijack import model_hijack
from backend.hypernetworks.modules.shared import opts, cmd_opts, state
import backend.hypernetworks.modules.shared as shared
import backend.hypernetworks.modules.face_restoration
import backend.hypernetworks.modules.images as images
...
def get_correct_sampler(p):
    if isinstance(p, backend.hypernetworks.modules.processing.StableDiffusionProcessingTxt2Img):
        return sd_samplers.samplers
    elif isinstance(p, backend.hypernetworks.modules.processing.StableDiffusionProcessingImg2Img):
        return sd_samplers.samplers_for_img2img
    elif isinstance(p, backend.hypernetworks.modules.api.processing.StableDiffusionProcessingAPI):
        return sd_samplers.samplers
...
def process_images_inner(p: StableDiffusionProcessing) -> Processed:
    ...
    backend.hypernetworks.modules.sd_hijack.model_hijack.apply_circular(p.tiling)
    backend.hypernetworks.modules.sd_hijack.model_hijack.clear_comments()
    ...
    with torch.no_grad(), p.sd_model.ema_scope():
        ...
        for n in range(p.n_iter):
            ...
            if opts.filter_nsfw:
                import backend.hypernetworks.modules.safety as safety
                x_samples_ddim = backend.hypernetworks.modules.safety.censor_batch(x_samples_ddim)
            ...
            for i, x_sample in enumerate(x_samples_ddim):
                ...
                if p.restore_faces:
                    ...
                    x_sample = backend.hypernetworks.modules.face_restoration.restore_faces(x_sample)
                    devices.torch_gc()
